...
# 
# Download the following dataset files, attach to a default binary system, and run **estimators** and **optimizers** to try to find a decent fit to the observations.  Later, we'll use the optimized solution as a starting point for MCMC - so try to get the residuals as flat as possible and make sure to save your progress.
# 
...
# 3. Only use one of the two provided light curves, but use the "known" spectral type of the _entire system_ of F9 as a _prior_.  You'll need to create a [custom parameter and constraint](http://phoebe-project.org/docs/2.3/tutorials/constraints_custom) to represent the system temperature and then decide how to set the spectral type as a distribution on that temperature.  This is a much more realistic case where the spectral type came from a combined spectrum or color temperature, but does require some manual setup and careful consideration of approximations.
# 4. Use both of the provided light curves and use them to extract absolute temperatures.  You'll need to make sure that they're coupled together so that only a _single pblum_ parameter (along with both temperatures or the temperature sum and ratio) is fitted.  Although simpler to setup than option \#3, this may require more time to converge and (in real life) requires callibrated photometry.
# 
...
# 
# For optimization runs, feel free to submit the jobs remotely and make use of `progress_every_niters` and `b.load_job_progress`.  Once you get somewhat close to a good solution, make sure to determine if any of the expensive effects can safely be disabled (see [optimizing computations](./Tutorial_optimizing_computations) from the first week) before adding a bunch of extra free parameters or running for extended iterations overnight.  For any approximations you do take, make sure to re-evaluate their validity as you go!
