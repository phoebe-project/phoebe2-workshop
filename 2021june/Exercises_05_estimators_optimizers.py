...
# 
# For optimization runs, feel free to submit the jobs remotely and make use of `progress_every_niters` and `b.load_job_progress`.  Once you get somewhat close to a good solution, make sure to determine if any of the expensive effects can safely be disabled (see [optimizing computations](./Tutorial_optimizing_computations.ipynb) from the first week) before adding a bunch of extra free parameters or running for extended iterations overnight.  For any approximations you do take, make sure to re-evaluate their validity as you go!
