...
# And load the [bundle from the previous tutorial](https://github.com/phoebe-project/phoebe2-workshop/raw/2025aug/data/synthetic/after_final_round.bundle):
...
# type of distribution to use when calling adopt_solution, get_distribution_collection, or plot. 
# 
# `mvsamples:` chains are stored directly and used for sampling with a KDE generated on-the-fly to compute probabilities.
# 
# `mvhistogram:` chains are binned according to distributions_bins and stored as an n-dimensional histogram.  
# 
# `mvgaussian:` a multivariate gaussian is fitted to the samples, use only if distribution is sufficiently represented by gaussians.  
# 
# `samples:` a univariate representation of mvsamples.  
# 
# `histogram:` a univariate representation of mvhistogram.  
# 
# `gaussian:` a univariate representation of mvgaussian.
# 
...
# PHOEBE uses the distl package that provides a python object-interface on top of several distribution (random variable) functions in scipy.stats. distl provides some convenient plotting functions:
